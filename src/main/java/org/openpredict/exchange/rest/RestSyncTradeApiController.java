...
public class RestSyncTradeApiController {
    ...
    public void initRestApi() {
        ...
        On.post("/syncTradeApi/v1/orders").json((Req req, RestApiPlaceOrder placeOrder) -> {
            ...
            return gatewayState.doAsyncCall(
                    req,
                    ticket -> api.placeNewOrder(ticket, longPrice, longSize, placeOrder.getAction(), placeOrder.getOrderType(), symbolId, placeOrder.getUid()));
        });
        ...
        On.put("/syncTradeApi/v1/orders").json((Req req, RestApiMoveOrder moveOrder) -> {
            ...
            return gatewayState.doAsyncCall(
                    req,
                    ticket -> api.moveOrder(ticket, longPrice, longSize, moveOrder.getOrderId(), symbolId, moveOrder.getUid()));
        });
        ...
        On.delete("/syncTradeApi/v1/orders").json((Req req, RestApiCancelOrder cancelOrder) -> {
            ...
            return gatewayState.doAsyncCall(
                    req,
                    ticket -> api.cancelOrder(ticket, cancelOrder.getOrderId(), symbolId, cancelOrder.getUid()));
        });
